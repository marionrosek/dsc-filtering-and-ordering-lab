...
```python
pd.read_sql("""SELECT name,breed FROM dogs WHERE gender='F';""",conn )
```
...
```python
pd.read_sql("""SELECT
            COUNT(*) AS no_dog_name 
            FROM dogs
            WHERE name IS NULL;""", conn)
```
...
```python
pd.read_sql("""SELECT name FROM dogs WHERE name LIKE '%ff%' OR '%oo%';""", conn )
```
...
```python
pd.read_sql("""SELECT name  FROM dogs ORDER BY name;""", conn)
```
...
```python
pd.read_sql("""SELECT name, age, temperament FROM dogs ORDER BY age DESC LIMIT 1;""", conn )
```
...
```python
pd.read_sql("""SELECT name, age FROM dogs ORDER BY age ASC LIMIT 3;""", conn)
```
...
```python
pd.read_sql("""SELECT name, breed FROM dogs WHERE age BETWEEN 5 AND 10 ORDER BY age DESC;""", conn)
```
...
```python
pd.read_sql("""SELECT name, age, hungry FROM dogs WHERE age BETWEEN 2 AND 7 ORDER BY name;""", conn)
```
